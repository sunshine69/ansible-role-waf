...
options:
    ...
    filters:
        ...
        - C(regex_pattern) is a dict with a C(name) key and C(regex_strings) list of strings to match
    purge_filters:
        description: Whether to remove existing filters from a condition if not passed in C(filters). Defaults to false
    state:
        description: Whether the condition should be I(present) or I(absent)
        choices:
        - present
        - absent
        default: present
...
#############################################################
# Unmerged changes from ansible.module_utils.aws.waf
#############################################################
...
#############################################################
# END of unmerged changes from ansible.module_utils.aws.waf
#############################################################
